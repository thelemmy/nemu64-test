...
use crate::cop0;
use crate::cop0::{
    make_entry_hi, make_entry_lo, Cause, CauseException, Coherency, Context, Pagemask, Status,
    XContext,
};
...
impl Test for LWAddressNotSignExtended {
    fn name(&self) -> &str {
        "LW with address not sign extended (64 bit)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            ...
            Context::from_virtual_address(p as u64 & 0xFFFFFFFF),
            "Context",
        )?;
        soft_assert_eq(
            exception_context.xcontext,
            XContext::from_virtual_address(p as u64 & 0xFFFFFFFF),
            "XContext",
        )?;
...
impl Test for SWAddressNotSignExtended {
    fn name(&self) -> &str {
        "SW with address not sign extended (64 bit)"
    }
...
fn test_load_and_catch_exception(address: u64, tlb_miss: bool) -> Result<(), String> {
    ...
    let cause_exception = if tlb_miss {
        CauseException::TLBL
    } else {
        CauseException::AdEL
    };
    ...
    soft_assert_eq(
        ...
        Cause::DEFAULT.with_exception(cause_exception),
        "Cause",
    )?;
    soft_assert_eq(
        exception_context.k0_exception_vector,
...
impl Test for TLBAndAddressError64 {
    fn name(&self) -> &str {
        "LW TLB Miss or Address Exception (64 bit addressing mode)"
    }
...
fn test_tlb_miss(address: u64, vpn: u27, r: u2) -> Result<(), String> {
    ...
    soft_assert_eq(
        exception_context.context,
        Context::DEFAULT.with_bad_vpn2(vpn19),
        "Context",
    )?;
    soft_assert_eq(
        exception_context.xcontext,
        XContext::DEFAULT.with_bad_vpn2(vpn27).with_r(r),
        "XContext",
    )?;
    soft_assert_eq(
        exception_context.entry_hi,
        make_entry_hi(0, vpn27, r),
        "EntryHi",
    )?;
...
impl Test for TLB64MissDueToR64Bit {
    fn name(&self) -> &str {
        "TLB: Expect TLB miss on R mismatch (64 bit addressing mode)"
    }
    ...
    fn values(&self) -> Vec<Box<dyn Any>> {
        vec![
            ...
            Box::new((
                0x00000001_F0000000u64,
                u27::new(0x003F_0000u32 >> 1),
                u2::new(0),
            )),
            Box::new((
                0x00000002_F0000000u64,
                u27::new(0x003F_0000u32 >> 1),
                u2::new(0),
            )),
            Box::new((
                0x0000007F_F0000000u64,
                u27::new(0x0FFF_0000u32 >> 1),
                u2::new(0),
            )),
        ]
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        match (*value).downcast_ref::<(u64, u27, u2)>() {
            Some((address, vpn, r)) => test_tlb_miss(*address, *vpn, *r),
            _ => Err("Value is not valid".to_string()),
        }
    }
}
