...
impl Test for DcacheLoadWordMatchesUncachedInit {
    fn name(&self) -> &str {
        "DCACHE: cached LW observes word written uncached"
    }
...
impl Test for DcacheStaleAfterUncachedStore {
    fn name(&self) -> &str {
        "DCACHE: cached LW stale after uncached SW until invalidate"
    }
...
impl Test for DcacheHitWritebackWritesDirtyToRam {
    fn name(&self) -> &str {
        "DCACHE: hit writeback pushes dirty line to RAM"
    }
...
impl Test for DcacheStoreTagThenLoadTag {
    fn name(&self) -> &str {
        "CACHE dcache store tag then load tag (TagLo)"
    }
...
impl Test for DcacheIndexWbInvClearsValidInTagLo {
    fn name(&self) -> &str {
        "CACHE dcache index writeback invalidate clears valid (TagLo via load tag)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        cache_common::run_cache_isolated_test(|| {
            ...
            soft_assert_eq(
                ...
                "TagLo after wb-invalidate + load tag",
            )?;
            Ok(())
        })
    }
...
impl Test for DcacheCreateDirtyExclusiveThenStore {
    fn name(&self) -> &str {
        "CACHE dcache create dirty exclusive (0x0D) then store/writeback"
    }
...
impl Test for DcacheHitWritebackInvalidate {
    fn name(&self) -> &str {
        "CACHE dcache hit writeback invalidate (0x15)"
    }
