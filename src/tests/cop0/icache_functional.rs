...
use crate::cop0;
use crate::cop0::{
    ICACHE_FILL, ICACHE_HIT_INVALIDATE, ICACHE_HIT_WRITEBACK, ICACHE_INDEX_INVALIDATE,
};
...
impl Test for IcacheFetchUsesMemoryImage {
    fn name(&self) -> &str {
        "ICACHE: fetch matches instruction words in RAM"
    }
...
impl Test for IcacheUncachedPatchStallsUntilInvalidate {
    fn name(&self) -> &str {
        "ICACHE: uncached patch does not affect fetched line until invalidate"
    }
...
impl Test for IcacheFillOpcodeLoadsLineFromRam {
    fn name(&self) -> &str {
        "CACHE icache fill (0x14) loads line from memory"
    }
...
impl Test for IcacheHitWritebackPushesLineToRam {
    fn name(&self) -> &str {
        "CACHE icache hit writeback (0x18) pushes line to RAM"
    }
