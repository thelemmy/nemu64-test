...
use crate::cop0::{
    self, make_entry_hi, make_entry_lo, CacheOp, CauseException, Coherency, Pagemask,
    RegisterIndex, Status, StatusKSU,
};
...
fn load_u32_sequence(register: GPR, value: u32) -> [u32; 2] {
    [
        Assembler::make_lui(register, (value >> 16) as i16),
...
fn load_u64_sequence(target: GPR, temp: GPR, value: u64) -> [u32; 8] {
    ...
    let lo = value as u32;
    [
        Assembler::make_lui(target, (hi >> 16) as i16),
        Assembler::make_ori(target, target, hi as u16),
        Assembler::make_dsll32(target, target, u5::new(0)),
        Assembler::make_lui(temp, (lo >> 16) as i16),
        Assembler::make_ori(temp, temp, lo as u16),
        Assembler::make_dsll32(temp, temp, u5::new(0)),
        Assembler::make_dsrl32(temp, temp, u5::new(0)),
...
impl EndianHarness {
    fn new() -> Self {
        Self {
            backing: UncachedHeapMemory::<u32>::new_with_align(
                TOTAL_WORDS,
                PAGE_BYTES * TOTAL_PAGES,
            ),
        }
    }

    fn setup_mappings(&mut self) {
        let paddr0 = self.backing.start_physical();
        ...
        unsafe {
            ...
            cop0::write_tlb(
                20,
                Pagemask::M4K,
                make_entry_lo(true, true, true, Coherency::Cached, (paddr0 >> 12) as u32),
                make_entry_lo(true, true, true, Coherency::Uncached, (paddr1 >> 12) as u32),
                make_entry_hi(0, u27::extract_u64(CODE_CACHED_BASE as u64, 13), u2::new(0)),
            );
            cop0::write_tlb(
                21,
                Pagemask::M4K,
                make_entry_lo(true, true, true, Coherency::Cached, (paddr2 >> 12) as u32),
                make_entry_lo(true, true, true, Coherency::Uncached, (paddr3 >> 12) as u32),
...
fn run_re_program(
    ...
    wrapped.push(Assembler::make_syscall() | (0x2e1 << 6));
...
fn run_re_program_labeled(
    label: &str,
    program: &[u32],
    cached_code: bool,
    mode_64bit: bool,
) -> Result<crate::exception_handler::ExceptionContext, String> {
    run_re_program(program, cached_code, mode_64bit)
        .map_err(|e| format!("{} [{}]: {}", label, re_mode_label(mode_64bit), e))
}

fn run_re_entry_labeled(
    label: &str,
    entry: u32,
    mode_64bit: bool,
) -> Result<crate::exception_handler::ExceptionContext, String> {
...
impl Test for ReExecuteCodePaths {
    fn name(&self) -> &str {
        "RE user mode execute from uncached and cached code pages"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for mode_64bit in [false, true] {
            ...
            let i2 = Assembler::make_sll(GPR::V0, GPR::V0, u5::new(1));
            let i3 = Assembler::make_addiu(GPR::V0, GPR::V0, 3);
            let i4 = Assembler::make_syscall() | (0x1f3 << 6);
            ...
            let ctx_cached =
                run_re_entry_labeled("execute cached code path", cached_entry, mode_64bit)?;
            let ctx_uncached =
                run_re_entry_labeled("execute uncached code path", uncached_entry, mode_64bit)?;
...
impl Test for ReLoadMatrix {
    fn name(&self) -> &str {
        "RE user mode loads: cached/uncached + COP1"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let load_cases = [
            ...
            LoadCase {
                name: "LWC1",
                emit: |rt: GPR, off: i16, base: GPR| {
                    Assembler::make_lwc1(FR::new_with_raw_value(rt.raw_value()), off, base)
                },
                bytes: 4,
                signed: true,
                cop1: true,
            ...
            LoadCase {
                name: "LDC1",
                emit: |rt: GPR, off: i16, base: GPR| {
                    Assembler::make_ldc1(FR::new_with_raw_value(rt.raw_value()), off, base)
                },
                bytes: 8,
                signed: false,
                cop1: true,
        ...
        for mode_64bit in [false, true] {
            for memory_kind in [MemoryKind::Cached, MemoryKind::Uncached] {
                for case in load_cases.iter() {
                    ...
                    let mut program = vec![
                        Assembler::make_lui(GPR::T0, (address >> 16) as i16),
                    ...
                    soft_assert_eq2(actual, expected, || {
                        format!("{} from {}", case.name, memory_kind.name())
                    })?;
                }
            }
        }
...
impl Test for ReStoreMatrix {
    fn name(&self) -> &str {
        "RE user mode stores: cached/uncached + COP1"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let store_cases = [
            ...
            StoreCase {
                name: "SWC1",
                emit: |rt: GPR, off: i16, base: GPR| {
                    Assembler::make_swc1(FR::new_with_raw_value(rt.raw_value()), off, base)
                },
                rt: GPR::T0,
                bytes: 4,
                value: 0x89abcdef,
            ...
            StoreCase {
                name: "SDC1",
                emit: |rt: GPR, off: i16, base: GPR| {
                    Assembler::make_sdc1(FR::new_with_raw_value(rt.raw_value()), off, base)
                },
                rt: GPR::T0,
                bytes: 8,
        ...
        for mode_64bit in [false, true] {
            for memory_kind in [MemoryKind::Cached, MemoryKind::Uncached] {
                for case in store_cases.iter() {
                    ...
                    program.push(Assembler::make_syscall() | (0x2e2 << 6));
...
impl Test for ReDcacheLineToggle {
    fn name(&self) -> &str {
        "RE dcache single-pass: fill(RE=1) then writeback(RE=0)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for mode_64bit in [false, true] {
            ...
            let line_before =
                harness.read_page_bytes(MemoryKind::Cached, DATA_OFFSET, cop0::DCACHE_LINE_BYTES);
            ...
            let re1_program = vec![
                ...
                status_re0_seq[0],
                status_re0_seq[1],
                Assembler::make_mtc0(GPR::T2, RegisterIndex::Status),
                Assembler::make_nop(),
                Assembler::make_nop(),
                Assembler::make_nop(),
                Assembler::make_nop(),
            ...
            let re0_program = vec![
                ...
                Assembler::make_cache(CacheOp::DataHitWriteBack, 0, GPR::T0),
                Assembler::make_syscall() | (0x2f1 << 6),
            ];
            ...
            for i in 1..cop0::DCACHE_LINE_BYTES {
                soft_assert_eq2(line_after[i], line_before[i], || {
                    format!(
                        "dcache byte {} changed after RE1->RE0 [{}]",
                        i,
                        re_mode_label(mode_64bit)
                    )
                })?;
...
impl Test for ReIcacheLineToggle {
    fn name(&self) -> &str {
        "RE icache single-pass: fill(RE=1) then writeback(RE=0)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for mode_64bit in [false, true] {
            ...
            let re1_program = vec![
                Assembler::make_lui(GPR::T0, (address >> 16) as i16),
                Assembler::make_ori(GPR::T0, GPR::T0, address as u16),
                Assembler::make_cache(CacheOp::InstructionFill, 0, GPR::T0),
                status_re0_seq[0],
                status_re0_seq[1],
                Assembler::make_mtc0(GPR::T2, RegisterIndex::Status),
                Assembler::make_nop(),
            ...
            let re0_program = vec![
                Assembler::make_cache(CacheOp::InstructionHitWriteBack, 0, GPR::T0),
                Assembler::make_syscall() | (0x2f3 << 6),
            ];
            let program = re1_then_re0_encode(&re1_program, &re0_program);
            let entry = harness.write_program(true, &program);
            run_mode_program_with_cop0(
            ...
            for i in 0..cop0::ICACHE_LINE_BYTES {
                soft_assert_eq2(line_after[i], line_before[i], || {
                    format!(
                        "icache byte {} changed after RE1->RE0 [{}]",
                        i,
                        re_mode_label(mode_64bit)
                    )
                })?;
...
fn run_unaligned_32bit_load_pairs(memory_kind: MemoryKind, mode_64bit: bool) -> Result<(), String> {
    ...
    let prog_lw = vec![
        Assembler::make_lui(GPR::T0, (base >> 16) as i16),
        Assembler::make_ori(GPR::T0, GPR::T0, base as u16),
        Assembler::make_lw(GPR::T1, 0, GPR::T0),
    ];
    let ctx_lw = run_re_program_labeled(
        &format!("unaligned base LW {}", memory_kind.name()),
        &prog_lw,
        true,
        mode_64bit,
    )?;
    let prog_lwl_lwr = vec![
        Assembler::make_lui(GPR::T0, (base >> 16) as i16),
        Assembler::make_ori(GPR::T0, GPR::T0, base as u16),
        Assembler::make_lui(GPR::T1, 0xbeefu16 as i16),
    ...
    soft_assert_eq2(ctx_lwl_lwr.t1 as u32, ctx_lw.t1 as u32, || {
...
fn run_unaligned_64bit_load_pairs(memory_kind: MemoryKind) -> Result<(), String> {
    ...
    let ctx_ld = run_re_program_labeled(
        &format!("unaligned base LD {}", memory_kind.name()),
        &prog_ld,
        true,
        true,
    )?;
    ...
    soft_assert_eq2(ctx_ldl_ldr.t1, ctx_ld.t1, || {
        format!("LDL/LDR pair on {}", memory_kind.name())
    })?;
    Ok(())
}

fn run_unaligned_32bit_store_pairs(
    ...
    let program_sw = vec![
        ...
        Assembler::make_syscall() | (0x2e3 << 6),
    ];
    let encoded_sw = re_fetch_encode(&program_sw);
    let entry_sw = harness_sw.write_program(true, &encoded_sw);
    let _ = run_re_entry_labeled(
        &format!("unaligned SW baseline {}", memory_kind.name()),
        entry_sw,
        mode_64bit,
    )?;
    if matches!(memory_kind, MemoryKind::Cached) {
    ...
    let bytes_sw =
        harness_sw.read_page_bytes(memory_kind, (eff_sw - memory_kind.data_base()) as usize, 8);
    ...
    let bytes_swl_swr = harness_swl_swr.read_page_bytes(
        memory_kind,
        (eff_swl_swr - memory_kind.data_base()) as usize,
        8,
    );
    for i in 0..8 {
...
fn run_unaligned_64bit_store_pairs(memory_kind: MemoryKind) -> Result<(), String> {
    ...
    let bytes_sd =
        harness_sd.read_page_bytes(memory_kind, (eff_sd - memory_kind.data_base()) as usize, 16);
    ...
    let bytes_sdl_sdr = harness_sdl_sdr.read_page_bytes(
        memory_kind,
        (eff_sdl_sdr - memory_kind.data_base()) as usize,
        16,
    );
    for i in 0..16 {
...
impl Test for ReUnaligned32BitLoads {
    fn name(&self) -> &str {
        "RE user mode unaligned 32-bit loads (LWL/LWR)"
    }
...
impl Test for ReUnaligned32BitStores {
    fn name(&self) -> &str {
        "RE user mode unaligned 32-bit stores (SWL/SWR)"
    }
...
impl Test for ReUnaligned64BitLoads {
    fn name(&self) -> &str {
        "RE user mode unaligned 64-bit loads (LDL/LDR)"
    }
...
impl Test for ReUnaligned64BitStores {
    fn name(&self) -> &str {
        "RE user mode unaligned 64-bit stores (SDL/SDR)"
    }
