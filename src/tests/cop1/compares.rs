...
use crate::cop1::{fcsr, set_fcsr, FCSRFlags, FCSRRoundingMode, FConst, FCSR};
...
fn test_compare<FIn: Copy>(
    ...
    value2: FIn,
    instruction: u32,
    expected: Result<(FCSRFlags, bool), ()>,
) -> Result<(), String> {
    ...
    if let Ok((expected_flags, expected_result)) = expected {
        ...
        for enabled in [FCSRFlags::NONE, expected_flags.invert()] {
            for condition_before in [false, true] {
                set_fcsr(
                    FCSR::ZERO
                        .with_condition(condition_before)
                ...
                soft_assert_eq(
                    result_fcsr,
                    expected_fcsr,
                    "FCSR after operation with exceptions disabled",
                )?;
            }
        }
    ...
    for condition_before in [false, true] {
        ...
        let expected_fcsr = expected_cause
            .with_condition(condition_before)
            .with_rounding_mode(rounding_mode)
            .with_enables(enabled_flags);
        soft_assert_eq(
...
fn test_compare_f32<const INSTRUCTION: u32>(
    rounding_mode: FCSRRoundingMode,
    value1: f32,
    value2: f32,
    expected: Result<(FCSRFlags, bool), ()>,
...
fn test_compare_f64<const INSTRUCTION: u32>(
    rounding_mode: FCSRRoundingMode,
    value1: f64,
    value2: f64,
    expected: Result<(FCSRFlags, bool), ()>,
...
fn test_impl<
    FMatch: Fn(Ordering, FPUSpecialNumber) -> Result<(FCSRFlags, bool), ()>,
    const INSTRUCTION_S: u32,
    const INSTRUCTION_D: u32,
>(
    value: &Box<dyn Any>,
    matcher: FMatch,
) -> Result<(), String> {
    match (*value).downcast_ref::<(f32, f32, Ordering, FPUSpecialNumber)>() {
        Some((value1, value2, ordering, special)) => {
            for rounding_mode in FCSRRoundingMode::ALL {
                test_compare_f32::<INSTRUCTION_S>(
                    rounding_mode,
                    *value1,
                    *value2,
                    matcher(*ordering, *special),
                )?;
            }
            return Ok(());
        }
    ...
    match (*value).downcast_ref::<(f64, f64, Ordering, FPUSpecialNumber)>() {
        Some((value1, value2, ordering, special)) => {
            for rounding_mode in FCSRRoundingMode::ALL {
                test_compare_f64::<INSTRUCTION_D>(
                    rounding_mode,
                    *value1,
                    *value2,
                    matcher(*ordering, *special),
...
impl Test for C_F {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::F, FR::F0, FR::F2);
...
impl Test for C_UN {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::UN, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_EQ {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::EQ, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_UEQ {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::UEQ, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_OLT {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::OLT, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_ULT {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::ULT, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_OLE {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::OLE, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();

        test_impl::<_, INSTRUCTION_S, INSTRUCTION_D>(value, |order, special| match special {
            FPUSpecialNumber::Nope => Ok((
                FCSRFlags::NONE,
...
impl Test for C_ULE {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::ULE, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_SF {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::SF, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_NGLE {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::NGLE, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_SEQ {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::SEQ, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_NGL {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::NGL, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_LT {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::LT, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_NGE {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::NGE, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_LE {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::LE, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
...
impl Test for C_NGT {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        const INSTRUCTION: FPUFloatInstruction =
            Assembler::make_cop1_c_cond(Cop1Condition::NGT, FR::F0, FR::F2);
        const INSTRUCTION_S: u32 = INSTRUCTION.s();
        const INSTRUCTION_D: u32 = INSTRUCTION.d();
