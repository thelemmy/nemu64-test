...
use core::any::Any;
use core::arch::asm;
use crate::cop0::{Cause, CauseException, preset_cause_to_copindex2, RegisterIndex, set_status, status, Status};
...
fn test_sw_interrupt(status_before: Status, cause: Cause, status_after: Status, fire_position: u32, hazard: bool) -> Result<(), String> {
    preset_cause_to_copindex2()?;
...
1:
    ...
    soft_assert_eq(exception_context.status, 0x2400_0003 | (cause.raw_value() & 0x300), "Status")?;
    ...
    if hazard {
        ...
        soft_assert_range(exception_context.exceptpc, expected_address - 4, expected_address, &"ExceptPC")?;
    }
...
impl Test for SoftwareInterrupt1Enabled {
    fn name(&self) -> &str { "SoftwareInterrupt1 (enabled)" }
...
impl Test for SoftwareInterrupt1Masked {
    fn name(&self) -> &str { "SoftwareInterrupt1 (masked)" }
...
impl Test for SoftwareInterrupt1InterruptsDisabled {
    fn name(&self) -> &str { "SoftwareInterrupt1 (interrupts disabled)" }
...
impl Test for SoftwareInterrupt2Enabled {
    fn name(&self) -> &str { "SoftwareInterrupt2 (enabled)" }
...
impl Test for SoftwareInterrupt2Masked {
    fn name(&self) -> &str { "SoftwareInterrupt2 (masked)" }
...
impl Test for SoftwareInterrupt2InterruptsDisabled {
    fn name(&self) -> &str { "SoftwareInterrupt2 (interrupts disabled)" }
...
impl Test for SoftwareInterrupt12Enabled {
    fn name(&self) -> &str { "SoftwareInterrupt12 (enabled)" }
...
impl Test for SoftwareInterrupt1EnabledHazard {
    fn name(&self) -> &str { "SoftwareInterrupt1 (enabled, hazard)" }
...
impl Test for SoftwareInterrupt1EnableAndDisableInstantly {
    fn name(&self) -> &str { "SoftwareInterrupt1 (enable but disable right away)" }
...
impl Test for SoftwareInterrupt1EnableAndDisableAfterOneNop {
    fn name(&self) -> &str { "SoftwareInterrupt12 (enable and disable after one nop)" }
