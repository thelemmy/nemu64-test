...
/// When using TLB with 4k pages it is possible for a dataCache line to contain the same
/// actual memory location (or two non-contiguous ones). This test ensures that the physicalAddress
/// is calculated through the ptag, NOT the cacheLineIndex
...
impl Test for CacheLineIndexPtagConflict {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        // Setup two 4k mappings from 0x0DEA0000 to data, both pointing to the beginning
        unsafe {
            cop0::write_tlb(
                0,
                Pagemask::M4K,
        ...
        data.write(2, 0x45);
        data.write(3, 0x67);
        data.write(4 * KB + 0, 0xBA);
        data.write(4 * KB + 1, 0xDB);
        data.write(4 * KB + 2, 0xAD);
        data.write(4 * KB + 3, 0xBA);
        ...
        unsafe {
            asm!("
                .set noat
                .set noreorder
                // This will load two cache lines, but they will come from the same physical address
                LW {out1}, 0 ({base})
                LW {out2}, 4096 ({base})

                // Write back both cache lines, but keep them cached
                LUI {temp}, 0xC0FF
                ORI {temp}, {temp}, 0xE000
                SW {temp}, 0({base})

                // Write back both cache lines, but keep them cached
                LUI {temp}, 0xF00D
                ORI {temp}, {temp}, 0x0000
                SW {temp}, 4096({base})

                // WriteBack and read the physical result
                CACHE {DataHitWriteBackInvalidate}, 0({base})
                LW {out3}, 0 ({physical_base})
                CACHE {DataHitWriteBackInvalidate}, 4096({base})
                LW {out4}, 0 ({physical_base})
            ",
                base = in(reg) 0x0DEA0000,
                physical_base = in(reg) MemoryMap::physical_to_uncached_mut::<u8>(data.start_physical()),
                temp = out(reg) _,
                out1 = out(reg) out0,
                out2 = out(reg) out1,
                out3 = out(reg) out2,
                out4 = out(reg) out3,
                DataHitWriteBackInvalidate = const DataHitWriteBackInvalidate as u32,
            )
        }

        soft_assert_eq(out0, 0x01234567, "Result value 1")?;
        soft_assert_eq(out1, 0x01234567, "Result value 2")?;
        soft_assert_eq(out2, 0xC0FFE000, "Value after write-back 1")?;
        soft_assert_eq(out3, 0xF00D0000, "Value after write-back 2")?;
