...
use crate::rdp::rdp::{
    DP_SET_STATUS_CLEAR_FREEZE, DP_SET_STATUS_CLEAR_XBUS, DP_SET_STATUS_SET_FREEZE,
    DP_SET_STATUS_SET_XBUS, DP_STATUS_COMMAND_BUFFER_READY, DP_STATUS_END_VALID, DP_STATUS_FREEZE,
    DP_STATUS_PIPE_BUSY, DP_STATUS_START_GCLK, DP_STATUS_START_VALID, DP_STATUS_XBUS, RDP,
};
...
fn wait_for_status(goal: u32) -> Result<(), String> {
    ...
    Err(format!(
        "Time out waiting for RDP status 0x{:x}. RDP status at timeout: 0x{:x}",
        goal,
        RDP::status()
    ))
}
...
impl Test for StartAndEndMasking {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            (RDP::status() & DP_STATUS_FREEZE) != 0,
            true,
            "RDP was told to freeze, but it didn't",
        )?;
        ...
        for value in [0xFFF, 0xFF_FFFF, 0x12FF_FFFF, 0x1280_0000, 0xFFFF_FFFF, 0] {
            ...
            RDP::set_start(value);
            RDP::set_end(value);
            soft_assert_eq2(RDP::start(), value & 0xFF_FFF8, || {
                format!("RDP START isn't masked properly on write (0x{:x} was written while end is 0x{:x})", value, RDP::end())
            })?;
            soft_assert_eq2(RDP::current(), value & 0xFF_FFF8, || {
                format!("RDP START isn't masked properly on write (0x{:x} was written while end is 0x{:x})", value, RDP::end())
            })?;
            soft_assert_eq2(RDP::end(), value & 0xFF_FFF8, || {
                format!(
                    "RDP END isn't masked properly on write (0x{:x} was written)",
                    value
                )
            })?;
        }
        ...
        soft_assert_eq(
            (RDP::status() & DP_STATUS_FREEZE) != 0,
            false,
            "RDP was told to stop being frozen but it's still frozen",
        )?;
...
impl Test for StartIsValidFlag {
    ...
    fn values(&self) -> Vec<Box<dyn Any>> {
        vec![Box::new(0x1238u32), Box::new(0u32)]
    }
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        match (*value).downcast_ref::<u32>() {
            Some(start_value) => {
                ...
                soft_assert_eq(
                    (RDP::status() & DP_STATUS_START_VALID) != 0,
                    false,
                    "start-valid should be false when entering the test",
                )?;
                soft_assert_eq(
                    (RDP::status() & DP_STATUS_END_VALID) != 0,
                    false,
                    "end-valid should be false when entering the test",
                )?;
                ...
                soft_assert_eq(
                    (status1 & DP_STATUS_START_VALID) != 0,
                    true,
                    "start-valid should be set after writing start-address",
                )?;
                soft_assert_eq(
                    (status1 & DP_STATUS_END_VALID) != 0,
                    false,
                    "end-valid should be false after writing start-address",
                )?;
                soft_assert_eq(start1, *start_value, "RDP start address after writing")?;

                soft_assert_eq(
                    (status2 & DP_STATUS_START_VALID) != 0,
                    true,
                    "Writing start after start should keep start-valid true",
                )?;
                soft_assert_eq(
                    (status2 & DP_STATUS_END_VALID) != 0,
                    false,
                    "Writing start while start-valid should not set end-valid",
                )?;
                soft_assert_eq(
                    start2,
                    *start_value,
                    "Writes to start address should be ignored while start-valid is set",
                )?;
                soft_assert_eq(
                    current2,
                    current_before,
                    "Writes to start should not affect current",
                )?;

                soft_assert_eq(
                    (status3 & DP_STATUS_START_VALID) != 0,
                    false,
                    "After writing end, start-valid should be cleared",
                )?;
                soft_assert_eq(
                    (status3 & DP_STATUS_END_VALID) != 0,
                    false,
                    "After writing end, end-valid should not be set (at least not while frozen)",
                )?;
                soft_assert_eq(
                    end3,
                    *start_value,
                    "Reading back end address after write to it (3)",
                )?;
                soft_assert_eq(
                    current3,
                    *start_value,
                    "Current should be equal to start (which is also equal to end) (3)",
                )?;
...
impl Test for StatusFlagsDuringRun {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let mut framebuffer = UncachedHeapMemory::<RGBA5551>::new_with_init_value(
            WIDTH * HEIGHT,
            32,
            RGBA5551::BLACK,
        );
        ...
        soft_assert_eq(
            RDP::start(),
            assembler.start() as u32,
            "RDP start should be equal to START after writing END",
        )?;
        soft_assert_eq(
            RDP::current(),
            assembler.start() as u32,
            "RDP current should be equal to START after writing END",
        )?;

        wait_for_status(
            DP_STATUS_COMMAND_BUFFER_READY | DP_STATUS_PIPE_BUSY | DP_STATUS_START_GCLK,
        )?;
        soft_assert_eq(
            RDP::start(),
            assembler.start() as u32,
        ...
        soft_assert_eq(
            framebuffer.read(0),
            RGBA5551::BLUE,
            "Auxiliary framebuffer should be filled with BLUE",
        )?;
        ...
        soft_assert_eq(
            RDP::start(),
            assembler.start() as u32,
            "RDP start should be equal to START after reaching the end",
        )?;
...
fn run_from_dmem_test<F: FnOnce(u32) -> (u32, u32)>(get_dmem_range: F) -> Result<(), String> {
    ...
    const HEIGHT: usize = 8;
    let mut framebuffer =
        UncachedHeapMemory::<RGBA5551>::new_with_init_value(WIDTH * HEIGHT, 32, RGBA5551::BLACK);
    ...
    soft_assert_eq(
        (dmem_end - dmem_start) & 0xFFF,
        length,
        "(end-start) must be equal to length",
    )?;
    ...
    unsafe {
        RDP::set_status(DP_SET_STATUS_SET_XBUS);
    }
    ...
    unsafe {
        RDP::set_status(DP_SET_STATUS_CLEAR_XBUS);
    }

    soft_assert_eq(
        RDP::current(),
        dmem_end,
        "RDP current should be equal to END after writing END (and waiting for the RDP to finish)",
    )?;

    soft_assert_eq(
        framebuffer.read(0),
        RGBA5551::GREEN,
        "Auxiliary framebuffer should be filled with GREEN",
    )?;
...
impl Test for RunFromDMEMEnd {
    fn name(&self) -> &str {
        "RDP STATUS: Run from DMEM (xbus) (end of dmem)"
    }
...
impl Test for RunFromDMEMOverflow {
    fn name(&self) -> &str {
        "RDP STATUS: Run from DMEM (xbus) (overflowing dmem)"
    }
