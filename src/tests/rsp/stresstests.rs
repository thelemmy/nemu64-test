...
use crate::rsp::rsp_assembler::{CP0Register, Element, RSPAssembler, E, GPR, VR};
use crate::rsp::spmem::SPMEM;
use crate::tests::soft_asserts::soft_assert_eq2;
use crate::tests::{Level, Test};
use crate::{MemoryMap, VIDEO};

fn zero_extend_accum48(v: i64) -> u64 {
    ((v as u64) << 16) >> 16
}

fn sign_extend_accum48(v: u64) -> i64 {
    ((v as i64) << 16) >> 16
}

#[inline(always)]
fn run_stress_test<F: Fn(&mut RSPAssembler) -> (), F2: Fn(u16, u16, u64) -> (u16, u64)>(
    name: &str,
    assembly_maker: F,
    cpu_computer: F2,
) -> Result<(), String> {
    ...
    for a_base in (0..=0xFFFF).step_by(8) {
        {
            let v = VIDEO.lock();
            {
                ...
                cursor.draw_text(
                    buffer,
                    format!(
                        "Stress testing {}. {:3.2}% complete",
                        name,
                        a_base as f32 / 65536.0f32 * 100.0f32
                    )
                    .as_str(),
                );
            }
        ...
        SPMEM::write(
            0x20,
            (&mut rsp_out_data[rsp_out_current_index]).as_mut_ptr() as u32,
        );
        RSP::start_running(0);
        for b_base in (0..=0xFFFF).step_by(STEPS_PER_RSP) {
            ...
            if (b_base as usize + STEPS_PER_RSP) <= 0xFFFF {
                SPMEM::write(
                    0x20,
                    (&mut rsp_out_data[rsp_out_next_index]).as_mut_ptr() as u32,
                );
                RSP::start_running(0);
            }
            ...
            for b_offset in 0..STEPS_PER_RSP {
                let rsp_result = unsafe {
                    MemoryMap::uncached(&rsp_out_data[rsp_out_current_index][b_offset][0])
                        .read_volatile()
                };
                let rsp_acc_high = unsafe {
                    MemoryMap::uncached(&rsp_out_data[rsp_out_current_index][b_offset][1])
                        .read_volatile()
                };
                let rsp_acc_mid = unsafe {
                    MemoryMap::uncached(&rsp_out_data[rsp_out_current_index][b_offset][2])
                        .read_volatile()
                };
                let rsp_acc_low = unsafe {
                    MemoryMap::uncached(&rsp_out_data[rsp_out_current_index][b_offset][3])
                        .read_volatile()
                };

                let b = b_base + b_offset as u16;
                for i in 0..8 {
                    let (result_val, result_accum) =
                        cpu_computer(a_base + i as u16, b, cpu_accumulator[i]);
                    cpu_accumulator[i] = result_accum;

                    let rsp_accum = ((rsp_acc_high.get16(i) as u64) << 32)
                        | ((rsp_acc_mid.get16(i) as u64) << 16)
                        | (rsp_acc_low.get16(i) as u64);
                    if (rsp_accum != result_accum) || (rsp_result.get16(i) != result_val) {
                        soft_assert_eq2(rsp_result.get16(i), result_val, || {
                            format!(
                                "Result vector for inputs 0x{:x} and 0x{:x}",
                                (a_base as usize) as i16,
                                b
                            )
                        })?;
                        soft_assert_eq2(rsp_accum, result_accum, || {
                            format!(
                                "Result accumulator for inputs 0x{:x} and 0x{:x}",
                                (a_base as usize) as i16,
                                b
                            )
                        })?;
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, _accum| {
                ...
                let result_val = if (temp > 0) && ((temp & !0x7FFFFFFF) != 0) {
                    0x7FFF
                } else {
                    (temp >> 16) as u16
                };
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, _accum| {
                ...
                let result_val = if (product_shifted & (1 << 48)) != 0 {
                    0
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, accum| {
                ...
                let result_val = if temp_shifted32 >= 0 {
                    if (temp_shifted32 & !0x7FFF) != 0 {
                        0x7FFFu16
                    } else {
                        temp_shifted32 as u16
                    }
                } else {
                    if (!temp_shifted32 & !0x7FFF) != 0 {
                        0x8000u16
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, accum| {
                let product = (a as i16 as i32) * (b as i16 as i32);
                let new_accum = ((product as i64) << 1) + sign_extend_accum48(accum);
                let result_val = if (new_accum & (1 << 48)) != 0 {
                    0
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, accum| {
                ...
                let result_val = if new_accum >= 0 {
                    if (new_accum & !0x7FFFFFFF) != 0 {
                        0xffff
                    } else {
                        new_accum as u16
                    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        run_stress_test(
            ...
            |a, b, accum| {
                ...
                let result_val = if product_shifted >= 0 {
                    if (product_shifted & !0x7FFF) != 0 {
                        0x7FFFu16
                    } else {
                        product_shifted as u16
                    }
                } else {
                    if (!product_shifted & !0x7FFF) != 0 {
                        0x8000u16
