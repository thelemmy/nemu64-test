...
impl Test for SetClearInterrupt {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            mi::is_sp_interrupt(),
            true,
            "MI INTR should contain SP after setting within SP_STATUS",
        )?;
        RSP::clear_interrupt();
        soft_assert_eq(
            mi::is_sp_interrupt(),
            false,
            "MI INTR should not contain SP after clearing within SP_STATUS",
        )?;
        RSP::set_interrupt();
        soft_assert_eq(
            mi::is_sp_interrupt(),
        ...
        RSP::set_status(
            crate::rsp::rsp::SP_STATUS_SET_SET_INTERRUPT
...
fn test_signal(i: u32) -> Result<(), String> {
    ...
    soft_assert_eq(
        RSP::is_signal(i),
        true,
        "SP_STATUS should contain signal after explicitly setting it (1)",
    )?;
    RSP::clear_signal(i);
    soft_assert_eq(
        RSP::is_signal(i),
        false,
        "SP_STATUS should not contain signal after explicitly clearing it (2)",
    )?;
    RSP::set_signal(i);
    soft_assert_eq(
        RSP::is_signal(i),
        true,
        "SP_STATUS should contain signal after explicitly setting it (3)",
    )?;
    ...
    soft_assert_eq(
        RSP::is_signal(i),
        true,
        "SP_STATUS should not change signal after setting and clearing it at once (4)",
    )?;
    ...
    soft_assert_eq(
        RSP::is_signal(i),
        false,
        "SP_STATUS should not contain signal after explicitly clearing it (5)",
    )?;
    ...
    soft_assert_eq(
        RSP::is_signal(i),
        false,
        "SP_STATUS should not change signal after setting and clearing it at once (6)",
    )?;
...
impl Test for SetClearInterruptOnBreak {
    fn name(&self) -> &str {
        "SP Set/Clear Interrupt on Break"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            true,
            "SP_STATUS should be INTERRUPT_ON_BREAK after explicitly setting it (1)",
        )?;
        RSP::clear_interrupt_on_break();
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            false,
            "SP_STATUS should not be INTERRUPT_ON_BREAK after explicitly clearing it (2)",
        )?;
        RSP::set_interrupt_on_break();
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            true,
            "SP_STATUS should be INTERRUPT_ON_BREAK after explicitly setting it (3)",
        )?;

        // Set and clear at the same time: No change expected
        RSP::set_status(
            crate::rsp::rsp::SP_STATUS_SET_SET_INTERRUPT_ON_BREAK
                | crate::rsp::rsp::SP_STATUS_SET_CLEAR_INTERRUPT_ON_BREAK,
        );
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            true,
            "If both INTERRUPT_ON_BREAK set and clear are set, nothing should change (4)",
        )?;
        ...
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            false,
            "SP_STATUS should not be INTERRUPT_ON_BREAK after explicitly clearing it (5)",
        )?;
        ...
        soft_assert_eq(
            RSP::is_interrupt_on_break(),
            false,
            "If both INTERRUPT_ON_BREAK set and clear are set, nothing should change (6)",
        )?;
...
impl Test for SetClearHalt {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        RSP::set_status(
            crate::rsp::rsp::SP_STATUS_SET_CLEAR_HALT
        ...
        soft_assert_eq(
            RSP::pc(),
            0x0,
            "If both SET_HALT and CLEAR_HALT are set, the RSP isn't supposed to run",
        )?;
...
impl Test for SPRegisterReadAccessOnRSP {
    fn name(&self) -> &str {
        "SP Register Read Access From RSP"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            SPMEM::read(0x4),
            0x20,
            "SP dram-address (read via RSP MFC0)",
        )?;
...
impl Test for SemaphoreRegisterCPUOnly {
    fn name(&self) -> &str {
        "SP Semaphore Register (CPU only)"
    }
...
impl Test for SemaphoreRegisterRSPOnly {
    fn name(&self) -> &str {
        "SP Semaphore Register (RSP only)"
    }
...
impl Test for SemaphoreRegisterMixed {
    fn name(&self) -> &str {
        "SP Semaphore Register (CPU and RSP)"
    }
...
impl Test for RSPHaltItselfWithoutBreak {
    fn name(&self) -> &str {
        "SP halt itself through Status.halt"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            RSP::status(),
            SP_STATUS_HALT,
            "RSP status after self-halt without BREAK",
        )?;
