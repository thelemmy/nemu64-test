...
fn run_test(
    ...
    soft_assert_eq(
        ...
        expected_result,
        "VMACF result when doing VMACF V6, V6, V1",
    )?;
    soft_assert_eq(
        SPMEM::read_vector16_from_dmem(0x150),
        expected_result,
        "VMACF result when doing VMACF V7, V0, V7",
    )?;
...
impl Test for VMACFAccumulatorOverflowed {
    fn name(&self) -> &str {
        "RSP VMACF (accumulator itself overflowed)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x100),
            [0x8000, 0x8000, 0, 0, 0, 0, 0, 0],
            "VMACF result after accumulator overflow in [0]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x110),
            [0x8000, 0x8001, 0, 0, 0, 0, 0, 0],
            "VMACF Acc[32..48] after accumulator overflow in [0]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x120),
            [0, 0, 0, 0, 0, 0, 0, 0],
            "VMACF Acc[16..32] after accumulator overflow in [0]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x130),
            [
            ...
            "VMACF Acc[0..16] after accumulator overflow in [0]",
        )?;

        // After second overflow
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x140),
            [0x8000, 0x7FFF, 0, 0, 0, 0, 0, 0],
            "VMACF result after accumulator overflow in [1]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x150),
            [0x8001, 0x7FFF, 0, 0, 0, 0, 0, 0],
            "VMACF Acc[32..48] after accumulator overflow in [1]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x160),
            [0x8000, 0x8003, 0, 0, 0, 0, 0, 0],
            "VMACF Acc[16..32] after accumulator overflow in [1]",
        )?;
        soft_assert_eq(
            ...
            "VMACF Acc[0..16] after accumulator overflow in [1]",
        )?;
