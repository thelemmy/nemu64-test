...
fn test<
    FEmit: Fn(&mut RSPAssembler, VR, i32, GPR, E),
    FSimulate: Fn(&Vector, &[Vector; 4], u32, E) -> [Vector; 4],
>(
    base_offset: usize,
    emit: FEmit,
    simulate: FSimulate,
    ...
    const REGISTER_DATA: Vector = Vector::from_u16([
        0x1776, 0x8378, 0xE1FE, 0x138F, 0xA42F, 0x156D, 0xCF20, 0x18E2,
    ]);

    // Prepare previous data: This is data that will be (partially) overwritten by the instruction
    const PREVIOUS_DATA: [Vector; 4] = [
        Vector::from_u16([
            0x1111, 0x1221, 0x1331, 0x1441, 0x1551, 0x1661, 0x1771, 0x1881,
        ]),
        Vector::from_u16([
            0x2112, 0x2222, 0x2332, 0x2442, 0x2552, 0x2662, 0x2772, 0x2882,
        ]),
        Vector::from_u16([
            0x3113, 0x3223, 0x3333, 0x3443, 0x3553, 0x3663, 0x3773, 0x3883,
        ]),
        Vector::from_u16([
            0x4114, 0x4224, 0x4334, 0x4444, 0x4554, 0x4664, 0x4774, 0x4884,
        ]),
    ];
    ...
    SPMEM::write_vector_into_dmem(
        0x510,
        &Vector::from_u16([
            0xBADB, 0xADBA, 0xDBAD, 0xBADB, 0xADBA, 0xDBAD, 0xBADB, 0xADBA,
        ]),
    );
    ...
    for misalignment in TEST_MISALIGNMENTS {
        for e in TEST_ELEMENTS {
            ...
            let expected = simulate(
                &REGISTER_DATA,
                &PREVIOUS_DATA,
                0x10 as u32 + misalignment,
                e,
            );
            for i in 0..4 {
                let address = (base_offset + i * 0x10 - 0x10) & 0xFFF;
                soft_assert_eq_vector(SPMEM::read_vector_from_dmem(address), expected[i], || {
                    format!(
                        "Store with e={:?} into memory location 0x{:x}. Checking address 0x{:x}",
                        e,
                        base_offset as u32 + misalignment,
                        address
                    )
                })?;
            }
...
fn test_simple<FEmit: Fn(&mut RSPAssembler, VR, i32, GPR, E), FGetMaxBytes: Fn(u32) -> u32>(
    base_offset: usize,
    emit: FEmit,
    get_maximum_bytes_from_offset: FGetMaxBytes,
) -> Result<(), String> {
    test(base_offset, emit, |source, previous_data, offset, e| {
        ...
        for i in 0..remaining_bytes_from_offset {
            ...
            result[result_vector_index]
                .set8(target_element_index, source.get8(source_element_index));
        }
        result
    })
}
...
impl Test for SBV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_simple(
            0x1000,
            |assembler, vt, offset, base, e| assembler.write_sbv(vt, e, offset, base),
            |_offset| 1,
        )
    }
}
...
impl Test for SSV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_simple(
            0x1000,
            |assembler, vt, offset, base, e| assembler.write_ssv(vt, e, offset, base),
            |_offset| 2,
        )
    }
}
...
impl Test for SLV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_simple(
            0x1000,
            |assembler, vt, offset, base, e| assembler.write_slv(vt, e, offset, base),
            |_offset| 4,
        )
    }
}
...
impl Test for SDV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_simple(
            0x1000,
            |assembler, vt, offset, base, e| assembler.write_sdv(vt, e, offset, base),
            |_offset| 8,
        )
    }
}
...
impl Test for SQV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_simple(
            0x1000,
            |assembler, vt, offset, base, e| assembler.write_sqv(vt, e, offset, base),
...
impl Test for SRV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for base_offset in [0x1000, 0xFF0, 0xFE0] {
            test(
                base_offset,
                |assembler, vt, offset, base, e| assembler.write_srv(vt, e, offset, base),
...
>(
    ...
    test(base_offset, emit, |source, previous_data, offset, e| {
        ...
        for i in 0..8 {
            ...
            let shift = if (element_index & 8) == 0 {
                SHIFT1
            } else {
                SHIFT2
            };
...
impl Test for SPV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_pack::<_, 8, 7>(0x1000, |assembler, vt, offset, base, e| {
            assembler.write_spv(vt, e, offset, base)
        })
    }
}
...
impl Test for SUV {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_pack::<_, 7, 8>(0x1000, |assembler, vt, offset, base, e| {
            assembler.write_suv(vt, e, offset, base)
        })
    }
}
        ...
        test(
            ...
            |source, previous_data, offset, e| {
                ...
                for i in 0..8 {
                    ...
                    let data16 = ((source.get8(element_index & 0xF) as u16) << 8)
                        | (source.get8((element_index + 1) & 0xF) as u16);
        ...
        for offset in TEST_OFFSETS {
            for vt in TEST_VT {
                for e in TEST_ELEMENT {
                    ...
                    const PREVIOUS_00: Vector = Vector::from_u16([
                        0xFFEE, 0xEEDD, 0xDDCC, 0xCCBB, 0xBBCC, 0xCCDD, 0xDDEE, 0xEEFF,
                    ]);
                    const PREVIOUS_10: Vector = Vector::from_u16([
                        0xBBAA, 0xAA99, 0x9988, 0x8877, 0x7788, 0x8899, 0x99AA, 0xAABB,
                    ]);
                    ...
                    for i in 0..16 {
                        let source_register_index = base_vt_index
                            + ((((i >> 1) as usize)
                                .wrapping_sub(((offset & !0x7) as usize) >> 1)
                                .wrapping_add(e.index() >> 1))
                                & 0x7);
                        ...
                        v.set8(
                            address & 15,
                            registers[source_register_index].get8(source_element_index & 15),
                        );
                    }
                    ...
                    // Verify result
                    soft_assert_eq_vector(SPMEM::read_vector_from_dmem(0x00), expected00, || {
                        format!("DMEM[0x000] after STV {:?}[{:?}] to 0x{:x}", vt, e, offset)
                    })?;
                    soft_assert_eq_vector(SPMEM::read_vector_from_dmem(0x10), expected10, || {
                        format!("DMEM[0x010] after STV {:?}[{:?}] to 0x{:x}", vt, e, offset)
                    })?;
                }
            }
        }
