...
fn test<F: Fn(&mut RSPAssembler, GPR, GPR)>(
    value: u32,
    expected_value: u32,
    write_shift: F,
) -> Result<(), String> {
    ...
    soft_assert_eq(
        SPMEM::read(0x8),
        0,
        "Any shift using R0 as source reg must result in 0",
    )?;
...
impl Test for SLL {
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        match (*value).downcast_ref::<(u32, u5, u32)>() {
            Some((source_value, shift_amount, expected_value)) => test(
                *source_value,
                *expected_value,
                |assembler, target_reg, source_reg| {
...
impl Test for SLLVWithShiftAmountOverwrite {
    fn name(&self) -> &str {
        "RSP SLLV (where target=shift amount)"
    }
...
impl Test for SRLVWithShiftAmountOverwrite {
    fn name(&self) -> &str {
        "RSP SRLV (where target=shift amount)"
    }
...
impl Test for SRAVWithShiftAmountOverwrite {
    fn name(&self) -> &str {
        "RSP SRAV (where target=shift amount)"
    }
