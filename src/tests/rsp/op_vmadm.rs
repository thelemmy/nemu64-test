...
fn run_test(
    ...
    soft_assert_eq(
        ...
        expected_result,
        "VMADM result when doing VMADM V6, V6, V1",
    )?;
    soft_assert_eq(
        SPMEM::read_vector16_from_dmem(0x150),
        expected_result,
        "VMADM result when doing VMADM V7, V0, V7",
    )?;
...
impl Test for VMADMAccumulatorOverflowed {
    fn name(&self) -> &str {
        "RSP VMADM (accumulator itself overflowed)"
    }
