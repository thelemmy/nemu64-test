...
impl Test for VRNDPOverwriteItselfWithElement {
    fn name(&self) -> &str {
        "RSP VRNDP (overwrite itself with element specifier)"
    }
...
impl Test for VRNDPAccumulatorOverflowed {
    fn name(&self) -> &str {
        "RSP VRNDP (accumulator itself overflowed)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x100),
            [0x8000, 0x8000, 0x7FFF, 0, 0, 0, 0, 0],
            "Result after accumulator overflow in [0]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x110),
            [0xffff, 0xc000, 0x7fff, 0, 0, 0, 0, 0],
        ...
        soft_assert_eq(
            ...
            [0x8000, 0x8000, 0x8000, 0, 0, 0, 0, 0],
            "Result after accumulator overflow in [3]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x150),
            [0xffff, 0xc000, 0x8000, 0, 0, 0, 0, 0],
            "Acc[32..48] after accumulator overflow in [3]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x160),
            [0x8000, 0x8000, 0x7ffd, 0, 0, 0, 0, 0],
            "Acc[16..32] after accumulator overflow in [3]",
        )?;
        soft_assert_eq(
            SPMEM::read_vector16_from_dmem(0x170),
            [0, 0, 0, 0, 0, 0, 0, 0],
            "Acc[0..16] after accumulator overflow in [3]",
        )?;
