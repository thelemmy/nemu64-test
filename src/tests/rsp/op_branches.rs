...
fn test<F: FnOnce(&mut RSPAssembler)>(
    value1_reg: GPR,
    value1: u32,
    value2: u32,
    expected_branch: bool,
    expected_ra_change: bool,
    write_branch: F,
) -> Result<(), String> {
    ...
    if expected_branch {
        soft_assert_eq(
            SPMEM::read(0x8),
            0,
            "Branch wasn't taken but it should have been",
        )?;
    } else {
        soft_assert_eq(
            SPMEM::read(0x8),
            1,
            "Branch was taken but it shouldn't have been",
        )?;
    }
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        if let Some((expected_branch, value1, value2)) = (*value).downcast_ref::<(bool, u32, u32)>()
        {
            test(
                ...
                *expected_branch,
                false,
                |assembler| assembler.write_beq(GPR::A0, GPR::A1, 2),
            )
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        if let Some((expected_branch, value1, value2)) = (*value).downcast_ref::<(bool, u32, u32)>()
        {
            test(
                ...
                *expected_branch,
                false,
                |assembler| assembler.write_bne(GPR::A0, GPR::A1, 2),
            )
        } else if let Some((expected_branch, value)) = (*value).downcast_ref::<(bool, u32)>() {
            // Single value means we compare against itself. This is a useful test for recompilers that might want to optimize this
            test(GPR::A0, *value, 0, *expected_branch, false, |assembler| {
