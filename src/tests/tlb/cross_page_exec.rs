...
fn install_split_tlb_result(
    buf: &mut UncachedHeapMemory<u32>,
    vaddr: u64,
    use_64: bool,
) -> Result<(), String> {
    let pfn0 = (buf.start_physical() >> 12) as u32;
    let pfn1 = ((buf.start_physical() + 2 * PAGE) >> 12) as u32;
    if pfn0 + 2 != pfn1 {
        return Err(format!(
            "expected third 4K PFN to be first+2, got pfn0={} pfn1={}",
            pfn0, pfn1
        ));
    }
    ...
    unsafe {
        ...
        cop0::write_tlb(
            11,
            Pagemask::M4K,
            make_entry_lo(true, true, true, Coherency::Uncached, pfn0),
            make_entry_lo(true, true, true, Coherency::Uncached, pfn1),
...
fn run_beq_delay(vaddr: u64, use_64: bool) -> Result<(), String> {
    ...
    write_u32(
        &mut buf,
        4096,
        Assembler::make_addiu(GPR::V0, GPR::V0, 0x100),
    );
    write_u32(
        &mut buf,
        4100,
        Assembler::make_addiu(GPR::V0, GPR::V0, 0x200),
    );
...
fn run_jr_delay(vaddr: u64, use_64: bool, jr_target: u64) -> Result<(), String> {
    ...
    let lo = tgt32 as u16;
    write_u32(&mut buf, 4084, Assembler::make_lui(GPR::T0, hi as i16));
    ...
    write_u32(
        &mut buf,
        4100,
        Assembler::make_addiu(GPR::V0, GPR::R0, 0x55),
    );
    write_u32(&mut buf, 4104, Assembler::make_jr(GPR::A0));
    write_u32(&mut buf, 4108, Assembler::make_nop());
    flush_icache_around(vaddr, use_64);
...
impl Test for CrossPageExecLinear32 {
    fn name(&self) -> &str {
        "TLB: linear icache across split 4K PFN (32-bit VA)"
    }
...
impl Test for CrossPageExecLinear64 {
    fn name(&self) -> &str {
        "TLB: linear icache across split 4K PFN (64-bit VA)"
    }
...
impl Test for CrossPageExecBeqDelay32 {
    fn name(&self) -> &str {
        "TLB: BEQ last in 4K, delay on next PFN (32-bit VA)"
    }
...
impl Test for CrossPageExecBeqDelay64 {
    fn name(&self) -> &str {
        "TLB: BEQ last in 4K, delay on next PFN (64-bit VA)"
    }
...
impl Test for CrossPageExecJrDelay32 {
    fn name(&self) -> &str {
        "TLB: JR last in 4K, delay on next PFN (32-bit VA)"
    }
...
impl Test for CrossPageExecJrDelay64 {
    fn name(&self) -> &str {
        "TLB: JR last in 4K, delay on next PFN (64-bit VA)"
    }
...
impl Test for CrossPageExecBranchInBranch32 {
    fn name(&self) -> &str {
        "TLB: branch in branch, inner in outer delay (32-bit VA)"
    }
...
impl Test for CrossPageExecBranchInBranch64 {
    fn name(&self) -> &str {
        "TLB: branch in branch, inner in outer delay (64-bit VA)"
    }
...
impl Test for CrossPageExecBranchInBranchInnerPage32 {
    fn name(&self) -> &str {
        "TLB: branch in branch, inner BEQ on next PFN (32-bit VA)"
    }
...
impl Test for CrossPageExecBranchInBranchInnerPage64 {
    fn name(&self) -> &str {
        "TLB: branch in branch, inner BEQ on next PFN (64-bit VA)"
    }
