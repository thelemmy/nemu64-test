...
pub fn setup_tlb_page(pagemask: Pagemask, valid: bool, dirty: bool) -> usize {
    ...
    unsafe {
        cop0::write_tlb(
            ...
            make_entry_hi(
                2,
                u27::extract_u64(virtual_page_base as u64, 13),
                u2::new(0),
            ),
        );
    ...
    F: FnOnce(usize) -> Result<(), String>,
{
    ...
    soft_assert_eq(
        ...
        0xFFFFFFFF_80000180,
        "Exception Vector for TLB exception",
    )?;
    ...
    soft_assert_eq(
        ...
        0xFFFFFFFF_80000000,
        "ExceptPC during TLB exception",
    )?;
    soft_assert_eq(
        exception_context.badvaddr,
        (virtual_page_base + offset) as u64,
        "BadVAddr during TLB exception",
    )?;
    soft_assert_eq(
        exception_context.cause,
        Cause::DEFAULT.with_exception(code).with_branch_delay(delay),
        "Cause during TLB exception",
    )?;
    soft_assert_eq(
        exception_context.status,
        0x24000002,
        "Status during TLB exception",
    )?;
    soft_assert_eq(
        exception_context.context.raw_value(),
        expected_context,
        "Context during TLB exception",
    )?;
    soft_assert_eq(
        exception_context.xcontext.raw_value(),
        expected_context,
        "XContext during TLB exception",
    )?;
    if check_entry_hi {
        // The docs say that asid on exception is the asid of the TLB entry, but test don't confirm that. It seems to stay unchanged
        soft_assert_eq(
            exception_context.entry_hi,
            make_entry_hi(
                1,
                u27::extract_u64(virtual_page_base as u32 as u64, 13),
                u2::new(0),
            ),
            "EntryHi during TLB exception",
        )?;
    }
...
pub fn test_nomiss_exception<F>(
    pagemask: Pagemask,
    offset: usize,
    valid: bool,
    dirty: bool,
...
impl Test for ReadMiss4k {
    fn name(&self) -> &str {
        "TLB: Read after 4k page, expect TLBL"
    }
...
impl Test for ReadMiss16k {
    fn name(&self) -> &str {
        "TLB: Read after 16k page, expect TLBL"
    }
...
impl Test for ReadMiss64k {
    fn name(&self) -> &str {
        "TLB: Read after 64k page, expect TLBL"
    }
...
impl Test for ReadMiss256k {
    fn name(&self) -> &str {
        "TLB: Read after 256k page, expect TLBL"
    }
...
impl Test for ReadMiss1M {
    fn name(&self) -> &str {
        "TLB: Read after 1M page, expect TLBL"
    }
...
impl Test for ReadMiss4M {
    fn name(&self) -> &str {
        "TLB: Read after 4M page, expect TLBL"
    }
...
impl Test for ReadMiss16M {
    fn name(&self) -> &str {
        "TLB: Read after 16M page, expect TLBL"
    }
...
impl Test for StoreMiss4k {
    fn name(&self) -> &str {
        "TLB: Store after 4k page, expect TLBS"
    }
...
impl Test for StoreMissMisaligned4k {
    fn name(&self) -> &str {
        "TLB: Store misaligned after 4k page, expect TLBS"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let swl = |address: usize| {
            unsafe {
                asm!("swl $0, 0($2)", in("$2") address);
            };
            Ok(())
        };
        let swr = |address: usize| {
            unsafe {
                asm!("swr $0, 0($2)", in("$2") address);
            };
            Ok(())
        };
        test_miss_exception(
...
impl Test for ExecuteTLBMapped4k {
    fn name(&self) -> &str {
        "TLB: Execute code mapped in 4k page"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        test_nomiss_exception(Pagemask::M4K, 4096, true, true, |address| {
            unsafe {
                ...
                ((address - 12) as *mut u32).write_volatile(Assembler::make_addiu(
                    GPR::V0,
                    GPR::R0,
                    0xFACEu16 as i16,
                ));
...
impl Test for ExecuteTLBMappedMiss {
    fn name(&self) -> &str {
        "TLB: Execute code that is not currently tlb mapped"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        soft_assert_eq(
            exception_context.cause,
            Cause::DEFAULT.with_exception(CauseException::TLBL),
            "Cause during TLB exception",
        )?;
        soft_assert_eq(
            exception_context.status,
...
impl Test for ExecuteTLBMappedMissInDelay {
    fn name(&self) -> &str {
        "TLB: Execute mapped branch with has a non-mapped delay slot"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let exception_context = expect_exception(CauseException::TLBL, -4i64 as u64, || {
            ...
            // and return
            unsafe {
                // The exception should resume us at -20 (ADDIU $2,$2,2)
                ((fault_address - 24) as *mut u32).write_volatile(Assembler::make_addiu(
                    GPR::V0,
                    GPR::V0,
                    1,
                ));
                ...
                // JALR $6, $3, with a delay slot in unmapped space
                ((fault_address - 4) as *mut u32)
                    .write_volatile(Assembler::make_jalr(GPR::A2, GPR::V1));
...
impl Test for TlbMissFetchAfterEret {
    fn name(&self) -> &str {
        "TLB: TLBL on first fetch after ERET (ExceptPC is EPC)"
    }
...
impl Test for ReadNonValid4k {
    fn name(&self) -> &str {
        "TLB: Read inside 4k page which isn't valid, expect TLBL"
    }
...
impl Test for ReadNonValid4kInDelay {
    fn name(&self) -> &str {
        "TLB: Read inside 4k page which isn't valid from a delay slot, expect TLBL"
    }
        ...
        test_miss_exception(
            ...
            |address| {
                ...
                soft_assert_eq(
                    unchanged,
                    0x12345678,
                    "LWC1 shouldn't overwrite target register as it should tlb fault",
                )?;
...
impl Test for StoreNonValid4k {
    fn name(&self) -> &str {
        "TLB: Store inside 4k page which isn't valid, expect TLBS"
    }
...
impl Test for StoreNonDirty4k {
    fn name(&self) -> &str {
        "TLB: Store inside 4k page which isn't dirty, expect Mod"
    }
...
impl Test for StoreNonDirtyAndNonValid4k {
    fn name(&self) -> &str {
        "TLB: Store inside 4k page which isn't dirty and not valid. TLBS wins over Mod"
    }
