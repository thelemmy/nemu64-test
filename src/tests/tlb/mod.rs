...
impl Test for IndexMasking {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for value in [
            0, 1, 5, 15, 30, 31, 32, 63, 64, 64, 1020, 63102, 0x0F000000, 0xFFFF0002, 0xFFFFFFFF, 0,
        ] {
            unsafe {
                cop0::set_index(value);
            }
            ...
            soft_assert_eq(
                ...
                expected,
                format!(
                    "Index was written as {} and came back as {} but was expected as {}",
                    value, readback, expected
                )
                .as_str(),
            )?;
        }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for value in [
            ...
            soft_assert_eq(
                ...
                expected,
                format!(
                    "EntryLo0 was written as {} and came back as {} but was expected as {}",
                    value, readback, expected
                )
                .as_str(),
            )?;
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for value in [
            ...
            soft_assert_eq(
                ...
                expected,
                format!(
                    "EntryLo1 was written as {} and came back as {} but was expected as {}",
                    value, readback, expected
                )
                .as_str(),
            )?;
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for value in [
            ...
            soft_assert_eq(
                readback,
                expected,
                format!("EntryHi was written as 0x{:x} via DMTC0", value).as_str(),
            )?;

            unsafe {
                cop0::set_entry_hi_32_64(value);
            }
            let expected = value & 0xC00000FF_FFFFE0FF;
            let readback = cop0::entry_hi();
            soft_assert_eq(
                readback,
                expected,
                format!("EntryHi was written as 0x{:x} via MTC0", value).as_str(),
            )?;
        }
        Ok(())
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for value in [
            ...
            soft_assert_eq(
                ...
                expected,
                format!(
                    "PageMask was written as {} and came back as {} but was expected as {}",
                    value, readback, expected
                )
                .as_str(),
            )?;
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        for value in [0x0000_8000, 0xF0FF_FFFF, 0x7006_E463] {
            ...
            soft_assert_eq(
                ...
                expected,
                format!(
                    "Config was written as {} and came back as {} but was expected as {}",
                    value, readback, expected
                )
                .as_str(),
            )?;
...
impl Test for TLBWriteReadPageMask {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let values_expected = [
            ...
            (
                0b00_11_00_01_10_10_01u32 << 13,
                0b00_11_00_00_11_11_00 << 13,
            ),
        ...
        for index in 0..values_expected.len() {
            unsafe {
                ...
                soft_assert_eq(
                    cop0::pagemask(),
                    values_expected[index].1,
                    format!(
                        "PageMask readback for index {} (0x{:x} written) unexpected",
                        index, values_expected[index].0
                    )
                    .as_str(),
                )?;
            }
    ...
    fn test(
        ...
        unsafe {
            ...
            soft_assert_eq(
                ...
                entry_lo0.1,
                format!(
                    "TLB readback for EntryLo0 (0x{:x} written) unexpected",
                    entry_lo0.0
                )
                .as_str(),
            )?;
            soft_assert_eq(
                ...
                entry_lo1.1,
                format!(
                    "TLB readback for EntryLo1 (0x{:x} written) unexpected",
                    entry_lo1.0
                )
                .as_str(),
            )?;
            soft_assert_eq(
                ...
                entry_hi.1,
                format!(
                    "TLB readback for EntryHi (0x{:x} written) unexpected",
                    entry_hi.0
                )
                .as_str(),
            )?;
...
impl Test for TLBWriteReadBackEntry {
    fn name(&self) -> &str {
        "EntryLo0/1/Hi after TLBWI/TLBR"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        self.test(
            0b00_00_00_00_00 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FFFFE0FF),
        )?;
        self.test(
            0b00_00_00_00_00 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0xC00000FF_FFFFE0FF, 0xC00000FF_FFFFE0FF),
        )?;
        self.test(
            0b00_00_00_00_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FFFF80FF),
        )?;
        self.test(
            0b00_00_00_11_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FFFE00FF),
        )?;
        self.test(
            0b00_00_11_11_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FFF800FF),
        )?;
        self.test(
            0b00_11_11_11_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FFE000FF),
        )?;
        self.test(
            0b11_11_11_11_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FF8000FF),
        )?;
        self.test(
            0b11_11_11_11_11 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0xC00000FF_FFFFE0FF, 0xC00000FF_FF8000FF),
        )?;
        ...
        self.test(
            ...
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x3FFFFFFF, 0x03FFFFFF),
            (0x00000000_FFFFE0FF, 0x00000000_FF8060FF),
        )?;
        self.test(
            0b11_11_11_10_01 << 13,
            (0x3FFFFFFF, 0x03FFFFFF),
...
impl Test for TLBUseTestRead0 {
    fn name(&self) -> &str {
        "TLB: Use and test, match via global (0)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        unsafe {
            cop0::write_tlb(
                ...
                make_entry_lo(
                    ...
                    false,
                    Coherency::Uncached,
                    (MemoryMap::HEAP_END >> 12) as u32,
                ),
                make_entry_lo(true, false, false, Coherency::Uncached, 0),
        ...
        unsafe {
            ...
            ((MemoryMap::HEAP_END_VIRTUAL_UNCACHED + 0x3FFC) as *mut u32)
                .write_volatile(0x12345678); // last value inside of the 16kb page
        }

        // Read it back using the TLB
        soft_assert_eq(
            unsafe { (0x0DEA0000 as *mut u32).read_volatile() },
            0xBADF00D,
            "Value read back through TLB mapped memory",
        )?;
        soft_assert_eq(
            unsafe { (0x0DEA0018 as *mut u32).read_volatile() },
            0xDECAFBAD,
            "Value read back through TLB mapped memory",
        )?;
        soft_assert_eq(
            unsafe { (0x0DEA3FFC as *mut u32).read_volatile() },
            0x12345678,
            "Value read back through TLB mapped memory",
        )?;
...
impl Test for TLBUseTestRead1 {
    fn name(&self) -> &str {
        "TLB: Use and test reading, match via global (1)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        // Read it back using the TLB
        soft_assert_eq(
            unsafe { (0x0DEA4000 as *mut u32).read_volatile() },
            0xBADF00D,
            "Value read back through TLB mapped memory",
        )?;
        soft_assert_eq(
            unsafe { (0x0DEA4018 as *mut u32).read_volatile() },
            0xDECAFBAD,
            "Value read back through TLB mapped memory",
        )?;
        soft_assert_eq(
            unsafe { (0x0DEA7FFC as *mut u32).read_volatile() },
            0x12345678,
            "Value read back through TLB mapped memory",
        )?;
...
impl Test for TLBUseTestReadMatchViaASID {
    fn name(&self) -> &str {
        "TLB: Use and test reading, match via ASID"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        unsafe {
            cop0::write_tlb(
                ...
                make_entry_lo(false, false, false, Coherency::Uncached, 0),
                make_entry_hi(1, u27::new(0xDEA0 >> 1), u2::new(0)),
            )
        }
        ...
        // Write a value without TLB
        unsafe {
            ((MemoryMap::HEAP_END_VIRTUAL_UNCACHED + 0x18) as *mut u32).write_volatile(0xCAFFEE11);
        }
...
impl Test for TLBPMatch {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA3), u2::new(1)));
        }
        cop0::tlbp();
        ...
        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA3), u2::new(0)));
        }
        cop0::tlbp();
        soft_assert_eq(
            cop0::index(),
            0x80000000,
            "TLBP result (match via ASID, incorrect R)",
        )?;

        // Match both global, with asid incorrect
        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0x7FF_0002), u2::new(0)));
        }
        cop0::tlbp();
        soft_assert_eq(
            cop0::index(),
            31,
            "TLBP result (ASID mismatch, but global is set)",
        )?;

        // Try to match with only one global on (with asid incorrect). This shouldn't match
        unsafe {
            cop0::set_entry_hi(make_entry_hi(0, u27::new(0x0001), u2::new(3)));
        }
        cop0::tlbp();
        soft_assert_eq(
            cop0::index(),
            0x80000000,
            "TLBP result (ASID mismatch, and only one of the global bits enabled)",
        )?;

        // Again, but this time the other global bit
        unsafe {
            cop0::set_entry_hi(make_entry_hi(0, u27::new(0x7FF_0000), u2::new(2)));
        }
        cop0::tlbp();
        soft_assert_eq(
            cop0::index(),
        ...
        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA0), u2::new(1)));
        }
        cop0::tlbp();
        soft_assert_eq(cop0::index(), 4, "TLBP result (masked VPN 1)")?;

        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA1), u2::new(1)));
        }
        cop0::tlbp();
        soft_assert_eq(cop0::index(), 4, "TLBP result (masked VPN 1)")?;

        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA2), u2::new(1)));
        }
        cop0::tlbp();
        soft_assert_eq(cop0::index(), 4, "TLBP result (masked VPN 2)")?;

        unsafe {
            cop0::set_entry_hi(make_entry_hi(1, u27::new(0xDEA4), u2::new(1)));
        }
        cop0::tlbp();
