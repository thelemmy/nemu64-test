...
impl Test for CartAddressMasking {
    fn name(&self) -> &str {
        "cart_memory: cart address (masking)"
    }
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for x in [0, 0xFFFFFFFF, 0xFEDCBA97] {
            ...
            soft_assert_eq(
                Pi::cart_address(),
                x & 0xFFFF_FFFE,
                "Cart address not masked correctly",
            )?;
            Pi::set_dram_address(x);
            soft_assert_eq(
                Pi::dram_address(),
                x & 0xFF_FFFE,
                "Dram address not masked correctly",
            )?;
        }
        Ok(())
    }
...
impl Test for PIDMA {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        ...
        Pi::set_status(
            PiStatusWrite::DEFAULT
                .with_reset(true)
                .with_clear_interrupt(true),
        );
        soft_assert_eq(Pi::status(), PiStatusRead::DEFAULT, "Pi Status before dma")?;

        let cart_addr =
            MemoryMap::physical_cart_address(&DATA[0] as *const u64 as *const u32) as u32;
        let mut target = UncachedHeapMemory::<u32>::new_with_init_value(100, 0, 0);
        let dram_address = target.start_physical() as u32;
        ...
        soft_assert_eq(
            Pi::cart_address() - cart_addr,
            16,
            "cart address after dma (difference to written value)",
        )?;
        soft_assert_eq(
            Pi::dram_address() - dram_address,
            16,
            "dram address after dma (difference to written value)",
        )?;
        soft_assert_eq(
            Pi::status(),
            PiStatusRead::DEFAULT.with_interrupt(true),
            "Pi Status after dma",
        )?;
        Ok(())
    }
...
fn test_pidma_transfer(
    rdram_misalign: isize,
    mut page_offset: isize,
    cart_addr: u32,
    requested_size: isize,
    expected: &[u8],
    cart_transfer_size: isize,
    rdram_transfer_size: isize,
) -> Result<(), String> {
    // Clear Pi, in case there's an error or interrupt pending
    Pi::set_status(
        PiStatusWrite::DEFAULT
    ...
    let dram_addr = target.start_physical() as u32 + rdram_misalign as u32 + page_offset as u32;
    ...
    for i in -16..0 {
        soft_assert_eq2(
            target.read((i + rdram_misalign + page_offset) as usize),
            0xaa,
            || format!("Prefix byte at offset {}", i),
        )?;
    }
    ...
    for i in actual_size..actual_size + 16 {
        soft_assert_eq2(
            target.read((i + rdram_misalign + page_offset) as usize),
            0xaa,
            || format!("Suffix byte at offset {}", i),
        )?;
    }

    soft_assert_eq2(
        (Pi::cart_address() - cart_addr) as isize,
        cart_transfer_size,
        || format!("cart address increment after DMA"),
    )?;
    soft_assert_eq2(
        (Pi::dram_address() - dram_addr) as isize,
        rdram_transfer_size,
        || format!("dram address increment after DMA"),
    )?;
    soft_assert_eq(
        Pi::status(),
        PiStatusRead::DEFAULT.with_interrupt(true),
...
impl Test for PIDMASmallSize {
    fn name(&self) -> &str {
        "cart_memory: DMA CART -> RDRAM (small sizes)"
    }
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let actual_size = if requested_size >= 126 {
            (requested_size + 1) & !1
        } else {
            requested_size
        };
...
impl Test for PIDMAMisaligned {
    fn name(&self) -> &str {
        "cart_memory: DMA CART -> RDRAM (misaligned)"
    }
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let actual_rom_size = if requested_size >= (126 - rdram_misalign as isize) {
            (requested_size + 1) & !1
        } else {
            requested_size
        };
        let actual_size = max(
            0,
            if actual_rom_size < 128 - rdram_misalign as isize {
        ...
        let expected: Vec<u8> = (0..actual_size)
            .map(|x| {
                if x >= 128 - 2 * rdram_misalign as isize && x < 128 - rdram_misalign as isize {
                    0xaa
                } else {
                    (x & 255) as u8
                }
...
impl Test for PIDMAMisalignedCrossPage {
    fn name(&self) -> &str {
        "cart_memory: DMA CART -> RDRAM (misaligned, cross page)"
    }
    ...
    fn run(&self, value: &Box<dyn Any>) -> Result<(), String> {
        ...
        let cart_addr =
            MemoryMap::physical_cart_address(&COUNTER16[0] as *const u16 as *const u32) as u32;
        let requested_size = size as isize;
        let actual_rom_size = if requested_size >= (64 - 2 - rdram_misalign as isize) {
            (requested_size + 1) & !1
        } else {
            requested_size
        };
        let actual_size = max(
            0,
            if actual_rom_size < 64 - rdram_misalign as isize {
                actual_rom_size - rdram_misalign as isize
            } else {
                actual_rom_size
            },
        ...
        let expected: Vec<u8> = (0..actual_size)
            .map(|x| {
                if x >= 64 - 2 * rdram_misalign as isize && x < 64 - rdram_misalign as isize {
                    0xaa
                } else {
                    (x & 255) as u8
                }
...
impl Test for PIDMAMisalignedEndOfPage {
    fn name(&self) -> &str {
        "cart_memory: DMA CART -> RDRAM (misaligned, end of page)"
    }
