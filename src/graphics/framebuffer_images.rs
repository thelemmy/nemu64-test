...
impl<T: Copy + Color> FramebufferImages<T> {
    ...
    pub fn alloc_buffers(&self, framebuffer_alignment: usize, width: u32, height: u32) {
        ...
        let framebuffer_layout =
            Layout::from_size_align(pixel_count * pixel_size, framebuffer_alignment).unwrap();

        let framebuffer1 = unsafe {
            Box::from_raw(slice::from_raw_parts_mut(
                alloc::alloc::alloc_zeroed(framebuffer_layout) as *mut T,
                pixel_count,
            ))
        };
    ...
    fn get_buffer(&self, front: bool) -> &Spinlock<Option<Image<T>>> {
        if front ^ self.buffer1_is_front() {
            &self.buffer2
        } else {
            &self.buffer1
        }
    ...
    pub fn frontbuffer(&self) -> &Spinlock<Option<Image<T>>> {
        self.get_buffer(true)
    }
    pub fn backbuffer(&self) -> &Spinlock<Option<Image<T>>> {
        self.get_buffer(false)
    }
}
